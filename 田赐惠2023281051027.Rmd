...
- 建筑形式由多到少为板楼，板塔结合，塔楼。平房非常少见。

- 单套房屋关注人数平均6.6人，中位数3人，最大值为262人。

- 地铁覆盖了超过一半的二手房

- 产证不满两年的房屋比满两年的更多，这类二手房在交易时会受到一些政策限制。
...
ggplot(data = lj, aes(x = price_sqm)) +
  ...
  labs(x="单价（元/平方米）",y="数量")
```

比较最热门的5个区域与全市平均房价，除了光谷东房价高于均值外，其他热门区域房价均低于全市均价，特别是盘龙城和金银湖，均价低于11000元。
```{r}
...
ggplot(data = price_distribution, aes(x = price_sqm, y = reorder(property_region, count), fill = property_region)) +  
  ...
  labs(title = "前五大区域房屋单价分布", x = "每平方米价格", y = "区域",fill="区域") +  
...
- 根据互联网搜索结果，板楼高度在12层以下，塔楼高度12-35层。但从链家数据来看，板楼在24层以上建筑中仍占主导。

- 精装二手房占多数，且比简装/毛坯房价格高15%左右。